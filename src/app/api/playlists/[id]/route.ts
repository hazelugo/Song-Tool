...
export async function GET(
  ...
  try {
    ...
    console.error(`GET /api/playlists/${params.id} error:`, err);
...
export async function DELETE(
  ...
  try {
    const [deleted] = await db
      ...
      .where(and(eq(playlists.id, params.id), eq(playlists.userId, USER_ID)))
      .returning();
    ...
    console.error(`DELETE /api/playlists/${params.id} error:`, err);
    return NextResponse.json(
